...
def get_all_dynatrace_entities(tenant, api_token, entity_selector, time=None, page_size=100):
    ...
    response = get_dynatrace_entity(tenant, api_token, time=time, page_size=page_size)
    ...
    while next_page is not None:
        # Get the next page of entities
        response = get_dynatrace_entity(tenant, api_token, time=time, page_size=page_size, next_page=next_page)
        # Loop through the entities
        for entity in response['values']:
            # Check if the entity matches the entity selector
            if entity_selector in entity['entityId']:
...
metrics: json = get_all_dynatrace_metrics(dynatrace_tenant, dynatrace_api_token)
